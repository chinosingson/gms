  ...
  <body classes="print print-page print-node-project print-node-<?php print $nid ?> <?php print $classes ?> gms print-pdf" >
		...
		<table id="print-pdf-container" border="0">
			...
			<tr class="print-pdf-table-row">
				<td colspan="2" class="col-sm-12" id="print-locations" >
					<h4 class="print-title">Locations</h4>
					<hr class="print-hr" />
					<?php
						//print print_r($node->field_project_locations['und'],1);
						if (isset($node->field_project_locations['und'])):
							$nodelocs = $node->field_project_locations['und'];
							$latLngs = array();
							foreach ($nodelocs as $latLngInfo){
								$latLngs[] = $latLngInfo['lat'].",".$latLngInfo['lng'];
							}
							$coords = implode("|",$latLngs);
							...
							print '<img id="project-map" src="https://maps.googleapis.com/maps/api/staticmap?size=700x350&scale=2&zoom=7&markers=icon:'.$iconurl.'|'.$coords.'">';
						else:
							print "This project has no specific locations.";
						endif;
					?>
				</td>
			</tr>
			<tr class="print-pdf-table-row">
				...
				<td class="pdf-col-2" id="print-pdf-funding">
					<div id="print-funding">
						...
						<div id="print-project-source-confinancing">Confinancing Source: <?php print $node->field_project_source_cofinancing['und'][0]['value']; ?></div>
						<?php endif; ?>
		...
		<table border="0" id="print-pdf-photos-table">
				<?php
					...
					foreach ($photos as $photo){
						...
						print "<td class=\"col-sm-6 print-image-caption\">".$photo['title']."<br/><span class='print-photo-source'>Source: ".$photo['alt']."</span></td>";
						print "</tr>";
					} ?>
		</table>
			<?php endif; ?>
