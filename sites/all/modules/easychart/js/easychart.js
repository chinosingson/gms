;(function($) {
  ...
  Drupal.behaviors.easychart = {
    attach: function() {
      ...
      // Open the popup and call the Easy Chart plugin.
      $('.easychart-configure-link').once('easyChart').click(function(e){
        e.preventDefault();
        var $widget = $(this).parents('.easychart-wrapper');

        // Create the Easy Chart plugin options based on the admin configuration.
        var options = {};
        ...
        if (Drupal.settings.easychart.unwantedOptions != '') {
          options.unwantedOptions = Drupal.settings.easychart.unwantedOptions;
        }
        if (Drupal.settings.easychart.unwantedReturnTypes != '') {
          options.unwantedReturnTypes = Drupal.settings.easychart.unwantedReturnTypes;
        }
        if (Drupal.settings.easychart.optionsStep1 != '') {
          options.optionsStep1 = Drupal.settings.easychart.optionsStep1;
        }
        if (Drupal.settings.easychart.optionsStep2 != '') {
          options.optionsStep2 = Drupal.settings.easychart.optionsStep2;
        }
        if (Drupal.settings.easychart.defaultColors != '') {
          options.defaultColors = Drupal.settings.easychart.defaultColors;
        }
